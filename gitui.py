...
class repoView (object):
    ...
    def tableview_cell_for_row(self, tableview, section, row):
        ...
        path_string = self.label_for_cell(section, row)
        max_string_length = 32
        if len(path_string) > max_string_length:
            path_string = path_string[:max_string_length] + '...'
        cell.text_label.text = path_string
        ...
        if section in (1, 2, 4, 5):
                        # dirty hack modify position
            b = ui.Button(frame=(cvf[2] - 32 * 1.5, 0, 32, cvf[3]))
...
editorpath = os.path.join(os.path.expanduser('~/Documents'), priority_list[0])
